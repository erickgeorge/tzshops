      ...
      <div class="row">
        <div class="col">
            <select name="problem_type" class="form-control mr-sm-2">
                ...
                <?php
                  $prob = WorkOrder::select('problem_type')->where('room_id',$_GET['workorders'])->OrWhere('location',$_GET['workorders'])->distinct()->get();
      ...
      <div class="row">
        <div class="col">
          <select name="location" class="form-control mr-sm-2">
                ...
                <?php
                  $loca = WorkOrder::select('location')->Where('location','<>',null)->where('room_id',$_GET['workorders'])->OrWhere('location',$_GET['workorders'])->distinct()->get();
...
//
  ...
  $userwithid = WorkOrder::select('client_id')->where('room_id',$_GET['workorders'])->OrWhere('location',$_GET['workorders'])->distinct()->get();
...
<option value="{{ $userfetch->id }}">{{ $userfetch->fname }} {{ $userfetch->lname }} - {{ $directora->name }}</option>
  ...
  <div class="modal-body">
    ...
    <?php $statusago = WorkOrder::select('status')->where('room_id',$_GET['workorders'])->OrWhere('location',$_GET['workorders'])->distinct()->get();
                    ...
                    @if($work->status !== 0)
                        ...
                        <tr>
                           ...
                           @elseif($work->status == 52)
                              ...
                              <td><span >IoW is checking for Work Order</span>
                                  <br>
                                @if($work->emergency == 1)
                                <span  >Emergency</span></td>
                                @endif
                           @elseif($work->status == 53)
                              ...
                              <td><span  >Work Order is not approved by IoW</span>
                                  <br>
                                @if($work->emergency == 1)
                                <span  >Emergency</span></td>
                                @endif
                          ...
                          @elseif($work->status == 25)
                              ...
                              <td><span  >Work Order Succesifully approved by IoW</span>
                                  <br>
                                @if($work->emergency == 1)
                                <span >Emergency</span></td>
                                @endif
