...
<div class="row container-fluid" >
  <div class="col">
    <h5 style=" text-transform: capitalize;">All Deactivated Users </b></h5>
...
<hr>
  ...
  <div class="row">
     <div class="col-md-3">
      <a href="{{ url('viewusers')}}" class="btn btn-primary" type="button">Active Users</a>
              ...
              @endforeach
                ...
                <div class="form-group" hidden >
                    ...
                    <select id="my-select" class="custom-select" name="typ">
                        ...
                        <?php
                            $allofem = User::select('type')->distinct()->where('status','0')->where('type','<>','')->orderBy('type','ASC')->get();
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <form method="GET" action="{{ url('exportdeactivatedusers') }}">
    <div class="modal-content">
      <div class="modal-header">
  ...
  <div class="modal-body">
        ...
        <?php
                 $userfetch = user::where('status','0')->get();
...
<option value="{{ $userfetch->id }}">{{ $userfetch->fname }} {{ $userfetch->mid_name }} {{ $userfetch->lname }} - ( {{ $departmentor->name }}, {{ $directora->name }})</option>
        ...
        <div class="row">
            <div class="col">
                <select name="type" class="form-control mr-sm-2">
                    ...
                    <option value="">All Types</option>
                    <?php
                    $type = User::select('type')->where('status','0')->distinct()->get();
      ...
      <div class="modal-body">
        <div class="row">
            <div class="col">
                <select name="directorate" class="form-control mr-sm-2" hidden>
                    <option selected="selected" value="">Select Directorate</option>
      ...
      <div class="modal-body">
        <div class="row">
            <div class="col">
                <select name="department" class="form-control mr-sm-2" hidden>
...
if ($_GET['page']==1){
 ...
 $i=1;
    ...
    <tr>
      ...
      @endif
        ...
        <a style="color: green;" href="{{ route('user.edit.view', [$user->id]) }}"  data-toggle="tooltip" title="Edit"><i class="fas fa-edit"></i></a>  &nbsp;
         ...
         <form  method="get" onsubmit="return confirm(' {{ $user->fname . ' '.$user->mid_name.' ' . $user->lname }} Will Be Activated')" action="{{ route('activateuser', [$user->id]) }}" >
          {{csrf_field()}}


        <button class="text-primary" type="submit" data-toggle="tooltip" title="Activate"   > <i class="fa fa-recycle" aria-hidden="true"></i>
  ...
  @endif
  @else
  <h2 style="text-align: center">No Deactivated Users Found</h2>
  @endif
</div>
