...
@endif
    ...
    <div class="row">
         <div class="col">
            <h5 ><b>Completed Works Order in  @if(auth()->user()->type == 'Maintenance coordinator')
      ...
      <div class="row">
        <div class="col">
            <select name="problem_type" class="form-control mr-sm-2">
                ...
                <?php
                  $prob = WorkOrder::select('problem_type')->distinct()->where('status',30)->orderBy('id','DESC')->get();
      ...
      <div class="row">
        <div class="col">
          <select name="location" class="form-control mr-sm-2">
                ...
                <?php
                  $loca = WorkOrder::select('location')->Where('location','<>',null)->distinct()->where('status',30)->orderBy('id','DESC')->get();
...
//
  ...
  $userwithid = WorkOrder::select('client_id')->distinct()->where('status',30)->orderBy('id','DESC')->get();
...
<option value="{{ $userfetch->id }}">{{ $userfetch->fname }} {{ $userfetch->lname }} - {{ $directora->name }}</option>
  ...
  <div class="modal-body">
    ...
    <?php $statusago = WorkOrder::select('status')->distinct()->where('status',30)->orderBy('id','DESC')->get();
