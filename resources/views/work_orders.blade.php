  ...
  <div class="col-md-6">
	...
	<form method="GET" action="work_order" class="form-inline my-2 my-lg-0">
      Start :  <input name="start" value="<?php
		 ...
		 echo $_GET['start']; } ?>" required class="form-control mr-sm-2" type="date" placeholder="Start Month" max="<?php echo date('Y-m-d'); ?>" >
	  End : <input value="<?php
		   if(request()->has('end')) { 
		 echo $_GET['end']; } ?>" 
	  name="end" required class="form-control mr-sm-2" type="date" placeholder="End Month" max="<?php echo date('Y-m-d'); ?>" >
