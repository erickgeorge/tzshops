...
<td style="color: blue"> <?php $tot = 0 -($item['material']->stock - $item->quantity); if($tot > 0){ echo 0 -($item['material']->stock - $item->quantity); }else{echo '-';}?></td>
  ...
  <div class="mine messages">
    <div class="message last">
     ...
     <b>{{ $conv->Message }}</b><br><small><?php $time = strtotime($conv->Sent); echo date('d/m/Y',$time);  ?></small>&nbsp;&nbsp;&nbsp; | From : <?php $huyu = User::Where('id',$conv->_From)->get(); ?>@foreach($huyu as $huyu) {{ $huyu->fname }} {{ $huyu->lname }}<hr>@if($huyu->signature_ == null) Not Signed <a href="{{ url('s-minutesheet') }}" class="btn btn-primary">Add signature</a> @else<img src="{{ $huyu->signature_ }}" alt="not signed" style="height: 100px;"> @endif @endforeach
     </div>
  </div>
    ...
    <b style="font-weight: bold;">{{ $conv->Message }}</b><br><small><?php $time = strtotime($conv->Sent); echo date('d/m/Y',$time);  ?></small>&nbsp;&nbsp;&nbsp; | From : <?php $huyu = User::Where('id',$conv->_From)->get(); ?>@foreach($huyu as $huyu) {{ $huyu->fname }} {{ $huyu->lname }}<hr><img src="{{ $huyu->signature_ }}" alt="not signed" style="height: 100px;">@endforeach
    </div>
  </div>
    @endif
    @endforeach
